...
pkgver=2449.0790dc99
_CLANG_VERSION_MAX=7.1
_CLANG_VERSION_MIN=7.0
pkgrel=1
...
sha256sums=('SKIP'
            ...
            'SKIP'
            'SKIP'
            'bc221a81c964da13c47fe9aabbcaff22c023c58ac46c167825f23e4fa2c40d13')
...
prepare() {
  ...
  git submodule init
  for _m in OmniSharpServer frozendict; do
    git config submodule.third_party/$_m.url "$srcdir/$_m"
    git submodule update third_party/$_m
  done
...
package() {
  ...
  cp -r "$srcdir/YouCompleteMe/third_party/ycmd/third_party/"{frozendict,tern_runtime} \
