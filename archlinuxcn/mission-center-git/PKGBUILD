...
source=("git+${url}.git"
	...
	"git+https://gitlab.com/${_pkgname}-devs/graph-widget.git"
...
prepare() {
	...
	git config submodule.subprojects/graph-widget.url "${srcdir}/graph-widget"
...
build() {
	meson setup --prefix=/usr --buildtype=release "${_pkgname}" build
