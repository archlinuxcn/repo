...
sha256sums=('8f887ea3e1bd9bc82e004d08b04dd5322b73294903c4543ef5fda48853bd3508')
...
package() {
	...
	# 安装文件
	install -Dm755 "build/linux/x64/release/bundle/${pkgname}" "${pkgdir}/opt/${pkgname}/${pkgname}"
	cp -a "build/linux/x64/release/bundle/lib" "${pkgdir}/opt/${pkgname}/"
	cp -a "build/linux/x64/release/bundle/data" "${pkgdir}/opt/${pkgname}/"

	# 设置库文件的 RPATH 为 $ORIGIN
	find "${pkgdir}/opt/${pkgname}/lib" -type f -name "*.so*" -exec \
