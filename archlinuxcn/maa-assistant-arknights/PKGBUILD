...
md5sums=('16d2ed62e26ee39d8f3b19ca5ca57dac'
         'SKIP'
...
prepare() {
    ...
    sed -e '/^find_package(fast/s/^/# /;' \
        -e '/maadeps/s/^/# /;' \
        -e 's/imgproc/imgproc calib videoio xfeatures2d/' \
...
_EOF
    ...
    sed -e 's/cv::BRISK/cv::xfeatures2d::BRISK/g;' \
        ...
        -i "${srcdir}/MaaAssistantArknights/src/MaaCore/Vision/FeatureMatcher.cpp"
}
...
build() {
    ...
    if ((WITH_CUDA)); then
        ...
        source /etc/profile # for variables in /etc/profile.d/cuda.sh
