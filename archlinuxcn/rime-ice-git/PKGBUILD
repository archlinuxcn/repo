...
build() {
    ...
    for _schema_name in "${_compile_schemas[@]}"; do
        rime_deployer --compile "$_schema_name.schema.yaml"
    done
...
_package_schema() {
    _schema=$1                                # 方案名
    _algebra=${2:-"algebra_${_schema}"}       # 启用的 algebra 方案
    _suggestion=${3:-${_schema}.default.yaml} # 默认配置源文件名
    ...
    if [[ -n "${_algebra}" ]]; then
        _patch_algebra "$_algebra" "$_install_base"
    fi
}
...
package_rime-ice-git() {
    ...
    for _schema_name in "${_schemas[@]}"; do
        _package_schema "${_schema_name}" "algebra_${_schemas[0]}" _default.yaml
    done
}
